...
export async function POST(req: NextRequest) {
  ...
  // Atomic max+create so a double-tap can't produce duplicate order values.
  const group = await prisma.$transaction(async (tx) => {
    const maxOrder = await tx.liftGroup.aggregate({ _max: { order: true } })
    return tx.liftGroup.create({
