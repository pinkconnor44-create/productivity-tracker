...
- **Calendar month cells** show kind-colored task pills (mobile: up to 2 pills, **start time only** — title hidden, untimed tasks show a tiny kind dot; sm:+ 2 pills with time prefix + truncated title; "+N more" footer when length > 2), score hairline at bottom, amber dot for notes — no full-cell tinted backgrounds, no MiniWheels. The 4-card stat strip (Day/Week/Month/Year) at the top replaces the old 3-wheel column. YearSpine on `xl:+` only. The pill must be a `<div>` (not `<span>`) with `overflow-hidden` so `truncate` actually clips against the column on narrow viewports. Use `max-w-full sm:w-full` on the pill itself so it sizes to its content on mobile (just the time fits naturally) but stretches to column width on desktop where the title needs the room to truncate. The parent column also uses `items-start` so pills don't get stretched by `align-items: stretch` default.
