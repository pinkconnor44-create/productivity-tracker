...
- **Calendar month + week cells** show kind-colored task pills. **Mobile (<sm)**: up to 3 pills, start time only via `shortTime()` helper (e.g., `9a`, `9:30a`, `12p`, `2:30p` — 2-6 chars max), no title, untimed tasks show a tiny kind dot. **Tablet+ (sm:)**: same pill but with truncated title visible. "+N more" footer when length > 3. Score hairline at bottom, amber dot for notes — no full-cell tinted backgrounds, no MiniWheels. The 4-card stat strip (Day/Week/Month/Year) at the top replaces the old 3-wheel column. YearSpine on `xl:+` only. The pill must be a `<div>` (not `<span>`) with `overflow-hidden` so `truncate` actually clips against the column on narrow viewports. Use `max-w-full sm:w-full` on month pills so they size to content on mobile but stretch on desktop where the title needs the room. Parent column uses `items-start` so pills don't get stretched by `align-items: stretch` default. Pill text-size scales with breakpoint: `text-[7px] sm:text-[10px]` with `px-0.5 sm:px-1 py-px` padding — at 7px font with `px-0.5`, a 6-char time like `12:30p` fits in ~25px usable cell width even on 320px (iPhone SE) viewports.
- **`shortTime(t)` helper** (`src/components/CalendarView.tsx`) is the canonical compact time format for narrow contexts. Use it instead of `formatTime()` anywhere a time has to fit in <50px. `formatTime()` stays for the day-detail view and tooltips where space isn't constrained.
