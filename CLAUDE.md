...
- **Calendar month cells** show kind-colored task pills (max 2 + "+N more"), score hairline at bottom, amber dot for notes — no full-cell tinted backgrounds, no MiniWheels. The 4-card stat strip (Day/Week/Month/Year) at the top replaces the old 3-wheel column. YearSpine on `xl:+` only.
